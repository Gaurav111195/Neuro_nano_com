# include<stdio.h>
			...
			int active_transport(int,int,int);
			int subgrad_receiver(int [], int [], int,int, int[]); 
			...
			node* start[250];
			node *temp;
			int	gap_index,i,arr_pkt,sw_car,sw_pkt,src,des,src_n,b[250],count,count2;
			int tot_arv_calcium=0,tot_arv_BDNF=0,tot_lost=0,tot_dep=0,tot_wait=0,value,Level,loss1,loss2,loss3,BDNF_arrive,int_loss1,int_loss2;
			...
			double time_ = 10;
			int wt[]={31,38,25,30,48,26};   	  // Q	uantity of each gate	
		        int val[]={25,21,18,29,35,23};  	 // time taken
		        int W;							    // total packet served
		        int n=6; 						   //No. of cargo
			int number_of_cargoes;
			int x;	
			double small_ABO_rate = 0.15*pow(10,-3)*5; //small pore
		    double large_ABO_rate = 1.5*pow(10,-3)*5; //large pore
			int no_of_pores = 500;
			double net_ABO_rate = no_of_pores * small_ABO_rate;
			int loss_subgrad;
			double fk;
			...
			int active_transport(int num_calcium, int num_BDNF, int no_of_stages)
			{
					// source to first level of tubules ( fick's law)     
			   	src_n =0 ;
			      for(src_n=0;src_n<no_of_stages;src_n++)
			      {
			        ...
			        for(arr_pkt=1;arr_pkt<=num_BDNF;arr_pkt++)
			           {
			              count=0;
			              temp= start[src_n]; 				    // checking input queue status
			              while(temp!=NULL)
			               {
			                 count++;
			                 temp=temp->next;
			    ...
			    for(Level=0;Level<1;Level++)
				  {
				    des=6*Level+no_of_stages-1;
				  for(src=6*Level;src<6*Level+no_of_stages;src++)
				   {
				     des++;
				     
				     calcium_concentration = diffusion_transport(src*10,tot_arv_calcium);
				     mean_sw_rate= (v_cargo/l_mt)-100*calcium_concentration; 
					 number_of_cargoes= mean_sw_rate*0.9+(mean_sw_rate*0.1)*(rand()%1000)/250;
			      ...
			      for(i=des-no_of_stages+1;i<=des;i++)
			         {
			         	while(start[i]!=NULL)
			         	{
			...
			double diffusion_transport(int distance,int initial_conc)
			{
				...
				double Deff = D/(1+B);
				double expo_term = -x*x/(4*Deff*time_);
				double constant_term = 1/(pow((4*3.142*Deff*time_),1.5));
			...
			int main()
			{
			   ...
			   for(int T=0;T<10;T++)
			   {
					...
					int number_of_BDNF_generated = transmitter(mean_arr_rate);
			   		int trans = active_transport(number_of_calcium_generated, number_of_BDNF_generated,6); 
			   }
			   		W= tot_dep;
					loss1 = tot_lost;
					BDNF_arrive = tot_arv_BDNF;
					
				int BDNF_transferred,BDNF_next;
														//1st stage after AIS
					tot_arv_calcium=0,tot_arv_BDNF=0,tot_dep=0,tot_lost=0,tot_wait=0,BDNF_transferred=0,tot_arv_calcium=0;
					for(int j =0;j<6;j++)
					BDNF_transferred += gates[j]*wt[j];
					int_loss2 = 0.02* BDNF_transferred;
					BDNF_next = floor(0.98 * BDNF_transferred); 
					for(int loop=0;loop<250;loop++)
			   {
			    start[loop]=NULL;
			    b[loop]=11000;
			   ...
			   {
			   	    int number_of_calcium_generated= transmitter(mean_arr_rate_ca);
					int trans = active_transport(number_of_calcium_generated, BDNF_next/60,6);
			   }
			   		W= tot_dep;
					loss2 = tot_lost;
														//2nd stage after AIS
					...
					for(int j =0;j<6;j++)
					BDNF_transferred += gates[j]*wt[j];
					int_loss1 = 0.02*BDNF_transferred;
					BDNF_next = 0.98 * BDNF_transferred; 
					for(int loop=0;loop<250;loop++)
			   {
			    start[loop]=NULL;
			    b[loop]=11000;
			...
			int metric_calculation()
			{
				...
				double Jx = c0*r11*exp(-r11*r11/(4*Diffusion_coefficient*time_))/(4*time_*sqrt(Diffusion_coefficient*time_*3.142));
				double Jy = c0*r11*exp(-y_distance*y_distance/(4*Diffusion_coefficient*time_))/(4*time_*sqrt(Diffusion_coefficient*time_*3.142));
			   ...
			   printf("\ntotal BDNF molecules arrived: %d",BDNF_arrive);
			   ...
			   int total_loss = loss1+loss2+loss3+int_loss1+int_loss2;
			   printf("\ntotal BDNF molecules lost in MT: %d\n",loss1+loss2+loss3+int_loss1+int_loss2);
			   printf("\ntotal BDNF molecules lost in optimisation part: %d\n",loss_subgrad);
			   printf("\ntotal BDNF molecules lost in whole model: %d\n",total_loss+loss_subgrad);
			   printf("\nblocking probability= %lf percent \n",100*((total_loss+loss_subgrad)*1.0/BDNF_arrive));
			   printf("\nMessage Deliverability = %lf percent \n",100-(100*((total_loss+loss_subgrad)*1.0/BDNF_arrive)));
			   printf("\nfunction = %g \n", fk );
 ...
 int subgrad_receiver(int val[], int wt[], int W,int n, int arr[])
{
	double zip=pow(10,20);
	double lambda1 = 21;
	double lambda2 = 120;//178;
	double lambda3 = 14;//23;
	double lambda4 = 33;//31;
	double lambda5 = 1122;//1399;
	double lambda6 = 125;
	int k=0,imp=0;
